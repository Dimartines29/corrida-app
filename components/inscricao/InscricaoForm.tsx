...
export function InscricaoForm() {
  ...
  const onSubmit = async (data: InscricaoCompleta) => {
    ...
    try {
      ...
      const pagamentoResponse = await fetch('/api/pagamento/criar-link-pagbank', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!pagamentoResponse.ok) {
        alert(
          ...
          `Porém houve um erro ao gerar o link de pagamento.\n` +
          `Você pode tentar novamente na área "Minha Área".\n\n` +
          `Erro: ${pagamentoResult.error}`
        );
      ...
      const checkoutUrl = pagamentoResult.checkoutUrl;

      if (!checkoutUrl) {
        alert(
          "Inscrição criada, mas o link de pagamento não foi gerado.\n" +
          "Por favor, acesse 'Minha Área' para finalizar o pagamento."
        );
        window.location.href = `/minha-area`;
        return;
      }
                ...
                <div className="flex flex-col sm:flex-row justify-between pt-4 sm:pt-6 gap-3 sm:gap-4">
                  ...
                  {currentStep < 5 ? (
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto bg-[#E53935] hover:bg-[#c62828] text-white font-bold px-6 sm:px-8 transition-all transform hover:scale-105 sm:ml-auto order-1 sm:order-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
